...
def supervisor_node(state: AgentState) -> AgentState:
    ...
    # Define keywords based on suggestions
    policy_tool_keywords = ["hoàn tiền", "refund", "flash sale", "license", "cấp quyền", "access level", "level 3", "emergency"]
    retrieval_keywords = ["p1", "escalation", "sla", "ticket"]
    human_review_keywords = ["err-", "không rõ"]

    # Keyword matching and routing logic
    if any(kw in task for kw in policy_tool_keywords):
        route = "policy_tool_worker"
        route_reason = "task contains policy or access control keyword"
        needs_tool = True
        if "emergency" in task or "khẩn cấp" in task:
             risk_high = True
             route_reason += " | emergency flagged risk_high"

    elif any(kw in task for kw in human_review_keywords):
        route = "human_review"
        route_reason = "task contains unknown error code or unclear context"
        risk_high = True
        
    elif any(kw in task for kw in retrieval_keywords):
        route = "retrieval_worker"
        route_reason = "task contains SLA or ticket keyword"
